...
class UpdatePhoneDetails extends StatefulWidget {
  final bool fromSettings;
  const UpdatePhoneDetails({Key? key, this.fromSettings = false})
      : super(key: key);

  @override
  _UpdatePhoneDetailsState createState() => _UpdatePhoneDetailsState();
}

class _UpdatePhoneDetailsState extends State<UpdatePhoneDetails>
    with FostrTheme {
                          ...
                          if (otpSent) {
                            if (_otpFormKey.currentState?.validate() ?? false) {
                              ...
                              try {
                                ...
                                // if (email != null &&
                                //     email.split('@')[1] == 'foster.com') {
                                //   auth.firebaseUser?.updateEmail(countryCode +
                                //       _controller.text +
                                //       '@foster.com');
                                // }

                                await auth.firebaseUser
                                    ?.updatePhoneNumber(cred);
                                FirebaseFirestore.instance
                                    .collection('users')
                                    .doc(auth.firebaseUser?.uid)
                                    .update({
                                  'phone': countryCode + _controller.text,
                                });
                                if (widget.fromSettings) {
                                  Navigator.of(context).pop();
                                } else {
